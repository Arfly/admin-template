...
async function getPostBindOrgs() {
  if (id) {
    const { code, data } = await getPostRelatedOrgList(id)
    if (code === SUCCESS_CODE) {
      treeSelectData.value = data.map((item: PostBindOrgListItem) => ({
        id: item.id,
      ...
      treeRef.value.setCheckedKeys(
        treeSelectData.value.map(item => item.id),
        true
      )
    }
  }
...
const submitUser = () => {
  formRef.value.validate(async valid => {
    ...
    let res
    if (id) {
      const { id, positionName, desc } = formObj.value
      res = await postEdit({
        id,
        positionName,
        desc,
