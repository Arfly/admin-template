...
import { SUCCESS_CODE, SysDetail, systemAdd } from '@/api'
import Remark from '@/components/common/Remark.vue'
import { NEW_SYSTEM_TOOLTIP } from '@/text'
import { NEW_SYS_VALIDATOR } from '@/validator'
...
const formRules = NEW_SYS_VALIDATOR
...
const submitUser = () => {
  formRef.value.validate(async valid => {
    ...
    delete params.id
    const res = await systemAdd({ ...params, adminPwd: pwd })
    code = res.code

    if (code === SUCCESS_CODE) {
      ElMessage.success('系统已成功创建')
    }
  })
}
</script>
